...
dataset = pd.read_csv(homefolder + '/datasets/tb_DTC_2nd_stage.csv',header=None)

dataset =  dataset.dropna()

X = np.array(dataset.iloc[1:, 0:5].values,dtype='float64')
y = np.array(dataset.iloc[1:, 4:6].values,dtype='float64')
X = np.delete(X, -1, axis=1)
parname = [ 'trf','nf_load','res','nf_dif' ]
...
reg = Sequential()
reg.add(Dense(units = 128, kernel_initializer = init.glorot_uniform(), activation = 'relu', input_dim = 4))
...
import pickle
name  = 'tb_DTC_2nd_stage'
addr = homefolder + '/reg_files/tb_DTC_2nd_stage/'
