...
dataset = pd.read_csv(homefolder+'/datasets/vcdl_28.csv',header=None)

dataset =  dataset.dropna()

X = np.array(dataset.iloc[1:, 0:6].values,dtype='float64')
y = np.array(dataset.iloc[1:, 5:9].values,dtype='float64')
X = np.delete(X, -1, axis=1)
parname = [ 'vcdl_n_width','vcdl_p_width','tc_n_width','tc_p_width','vdd']
...
reg = Sequential()
reg.add(Dense(units = 32, kernel_initializer = init.glorot_uniform(), activation = 'elu', input_dim = 5))
reg.add(Dense(units = 64, kernel_initializer = init.glorot_uniform(), activation = 'elu'))
reg.add(Dense(units = 64, kernel_initializer = init.glorot_uniform(), activation = 'elu'))
reg.add(Dense(units = 32, kernel_initializer = init.glorot_uniform(), activation = 'elu'))
...
reg.fit(sX_train, sy_train, batch_size = 128, epochs = 500)
...
import pickle
name  = 'vcdl_28'
addr = homefolder + '/reg_files/vcdl_28/'
