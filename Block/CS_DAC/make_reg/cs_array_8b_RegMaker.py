...
dataset = pd.read_csv(homefolder + '/datasets/PY_cs_array_8b_TT.csv',header=None)

dataset =  dataset.dropna()

X = np.array(dataset.iloc[1:, 0:5].values,dtype='float64')
y = np.array(dataset.iloc[1:, 5:8].values,dtype='float64')
...
reg = Sequential()
reg.add(Dense(units = 128, kernel_initializer = init.glorot_uniform(), activation = 'relu', input_dim = 5))
...
import pickle
name  = 'cs_array_8b'
addr = homefolder + '/reg_files/cs_array_8b/'
