...
class CK(TF_DEFAULT):
    ...
    def default_loading(self):
        if self.tech==65:
            ...
            self.minx  = np.array([[0,0.5e-6,2e-6,2e-6,400]])
            self.maxx  = np.array([[1,2e-6,6e-6,3e-6,700]])
            self.step  = np.array([[1,0.5e-6,2e-6,0.5e-6,50]])
...
#    y_sh1=sh1.tf_rescaley(sy_sh1)
    ...
    constraints = []    
    constraints.append(tf.nn.elu(  -1* (specs[0] -  9e9  )*10. ))     # Output Frequency constaint
    constraints.append(tf.nn.elu(  -1* (9.2e9   - specs[0] )*10. ))     # Output Frequency constaint
