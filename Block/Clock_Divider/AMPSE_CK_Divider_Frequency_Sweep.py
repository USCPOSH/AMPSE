...
class CK(TF_DEFAULT):
    ...
    def default_loading(self):
        if self.tech==65:
            drive       = home_address+'/reg_files/CK/'
            sx_f        = drive + '/scX_ckdiv65.pkl'
            sy_f        = drive + '/scY_ckdiv65.pkl'    
            w_f         = drive + '/w8_ckdiv65.p'
            self.w_json = drive + '/model_ckdiv65.json'
            self.w_h5   = drive + '/reg_ckdiv65.h5'
            self.minx  = np.array([[1,0.5e-6,3e-6,2e-6,400]])
            self.maxx  = np.array([[1,3e-6,5e-6,3e-6,800]])
            self.step  = np.array([[1,0.1e-6,0.2e-6,0.2e-6,50]])
...
def graph_tf(sxin,ck1,sh1):
    ...
    sx_ck1 = sxin[:,0:5]
    x_ck1=ck1.tf_rescalex(sx_ck1)
    sy_ck1=ck1.tf_reg_elu(sx_ck1)
    y_ck1=ck1.tf_rescaley(sy_ck1)
...
#
#    chosen=np.array([0,0,0,0,0])
#    vars_sh1 = sxin[:,4:9] # from the SH itself 
    
#    print(x_lna1[0,2])
#    print(y_lna1[0,0])
##    print(y_lna1[0,1])
#    cnst_sh1=sh1.tf_scalex2(tf.stack([x_ck1[0,0],y_ck1[0,0],y_ck1[0,0]],axis=0), 1-chosen) # from the LNA
#    
##    print(cnst_sh1)
##    print(vars_sh1)
#    
...
#    y_sh1=sh1.tf_rescaley(sy_sh1)
    ...
    output_freq    = y_ck1[0,0]; 
    power = y_ck1[0,1]; 
    out_max       = y_ck1[0,2]; 
    out_min    = y_ck1[0,3];    
    
    specs = []                                  
    specs.append(output_freq)         #   0-Output Frequency
    specs.append(power)               #   1- Total Power         
    specs.append(out_max)             #   2-Output High Voltage              
    specs.append(out_min)             #   3-Output Low Voltage    
                  
    constraints = []    
    constraints.append(tf.nn.elu(  -1* (specs[0] -  11.9e+09)*10. ))     # Output Frequency constaint
    constraints.append(tf.nn.elu(  -1* (12.1e+09   - specs[0] )*10. ))     # Output Frequency constaint
    constraints.append(tf.nn.elu(  -1* (specs[2]) ))                    # Maximize the Output High Value
    constraints.append(tf.nn.elu(  (specs[3]) ))                    # Minimize the Output Low Value
    constraints.append(tf.nn.elu(      (specs[1]) ))                    # Minimize the Total power consumption    
...
#                            specs   parameters     metrics
    return hardcost,usercost,specs,[x_ck1],[y_ck1],constraints
...
if __name__ == '__main__':
    ...
    sxin = make_var("RF_FE", "CK", (1,9), tf.random_uniform_initializer(-np.ones((1,9)),np.ones((1,9))))
    hardcost,usercost,tf_specs,tf_params,tf_metrics,tf_const = graph_tf(sxin,ck1,sh1)
