...
class Folded_Cascode(TF_DEFAULT):
    ...
    def default_loading(self):
		# load your previously saved regressor here
        if self.tech==65:
			...
			# copy your parameter range from the netlist database file
...
def param_to_sxin(param,folded_cascode,classab):
	...
	# scaled parameters for the regressor
...
def graph_tf(sxin,folded_cascode,classab):
    ...
    # add if you have any constants here 
    c_cload = tf.constant(cload,dtype = tf.float32)
    #----------Folded Cascode's graph----------
	# rescale the parameter and metric
    ...
    #--------Other variables--------
	# rescale compensation resistor and capacitor
	...
	# write your final metric using module level metric, require designer's knowledge
    ...
    specs.append(tf.math.atan(ugb_approx/zero_1) - tf.math.atan(ugb_approx/pole_2) - tf.math.atan(ugb_approx/pole_1) + 3.1416) # 2- Phase margin of the amplifier  +
    ...
    # put constraints on your final metric using desired spec
    constraints = []    
	...
	# satisfy all specs with minimum power consumption
