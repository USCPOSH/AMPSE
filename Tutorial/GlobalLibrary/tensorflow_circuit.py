...
def make_var(var_scope, var_name, var_shape, var_init):
    with tf.compat.v1.variable_scope(var_scope, reuse=tf.compat.v1.AUTO_REUSE):
        return tf.compat.v1.get_variable(shape=var_shape,dtype=tf.float32,name=var_name,initializer=var_init)
...
#        sparam=np.zeros_like(param)
        for i in range(len(param)):
            
            param[i,param[i,:]<self.minx[0]]=self.minx[0,param[i,:]<self.minx[0]]
            param[i,param[i,:]>self.maxx[0]]=self.maxx[0,param[i,:]>self.maxx[0]]        
        sparam = np.round(param/self.step)*self.step 
        return sparam
